...
def scan_and_store(app):
    ...
    net = ipaddress.ip_network("192.168.6.0/24")  # Exemplo para testar a rede local ispgaya
    ...
    def _ping_scan():
        args = ["nmap", "-sn", "-oX", "-", str(net)] #paramentros nmap -sn -oX - 192.168.6.0/24
    ...
    def _scan_ip(ip):
        ...
        # args original e funcional
        #args = ["nmap", "-sS", "-sV", "-T4", "-p", "1-1000", "-oX", "-", ip] 
       
        #args de teste para scan de vuln comos parametros sudo nmap -sS -sV --script vuln -T4 -p 1-1000 -oX - ip
    ...
    #versao editada para tratar erros
    def _scan_ip(ip):
    ...
    with ThreadPoolExecutor(max_workers=20) as exe:
       # roots = list(exe.map(_scan_ip, active_ips)) -> ORIGINAL E FUNCIONAL
        roots = list(exe.map(_scan_ip, active_ips))
        roots = [r for r in roots if r is not None]  # remove falhas
